  ...
  /* Since we're only using this for Temperature
   ...
   for(j=0;j<E->tracer.tr_in_element_number[E->mesh.levmax][el];j++) {
    ...
    /*RAA: 09/07/03 - Latent heat of melting stuff - next 2 lines from Craig's 2D routine*/
	 if(E->tracer.dFdot[m] > 0) {
	 /*fprintf(stderr,"Adv_diff: Qt %g Latent %g dFdot %g T %g fn %g Cp %g \n",QtE,E->tracer.Latent_heat[E->tracer.property_group[m]],E->tracer.dFdot[m],E->tracer.T[m],E->tracer.tracer_weight_fn[E->mesh.levmax][m],E->tracer.Cp[E->tracer.property_group[m]]); */
	...
	/* fprintf(stderr,"Adv_diff_2: Qt %g \n",QtE); */
	}
...
{ 
  ...
  standard_precision *divU; /*Rey: 27/04/07*/
  ...
  divU = (standard_precision *) Malloc0((1+E->mesh.nel) * sizeof(standard_precision)); /*Rey: 27/04/07*/
