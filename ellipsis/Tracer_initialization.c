			 ...
			 )
{
  ...
  for(m=0;m<=E->tracer.NUM_MATERIALS;m++) {
    ...
    sprintf(default_str,"Material_%d_melt_extract",m);
    input_std_precision(default_str,&(E->tracer.Melt_extract[m]),"0.0");
			     ...
			     )
{
  ...
  */
  ...
  Realloc0(&(E->tracer.strd_hor),old_allocation,current_allocation+10,sizeof(standard_precision));
  Realloc0(&(E->tracer.strd_ver),old_allocation,current_allocation+10,sizeof(standard_precision));
  ...
  Realloc0(&(E->tracer.melt),old_allocation,current_allocation+10,sizeof(standard_precision));
  ...
  for(i=former_highest_tracer+1;i<=current_highest_tracer;i++) {
	...
	E->tracer.strd_hor[i] = 0.0;
    E->tracer.strd_ver[i] = 0.0;
  ...
  */
  ...
  add_variable_to_tracer_output(E,E->tracer.Visc,"Pvisc");
  ...
  add_variable_to_tracer_output(E,E->tracer.melt,"Melt");
