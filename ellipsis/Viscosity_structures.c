  ...
  */
  ...
  for(material=0;material<=E->tracer.NUM_MATERIALS;material++) {
   ...
   /* sprintf(default_str,"Material_%d_melting",material); 
    ...
    sprintf(default_str,"Material_%d_melt_on",material);
    input_std_precision(default_str,&(E->tracer.Melt_on[material]),"0");
    ...
    sprintf(default_str,"Material_%d_depl_visc_change",material);
    input_std_precision(default_str,&(E->tracer.Depl_visc_change[material]),"0.0");
    sprintf(default_str,"Material_%d_melt_total_visc_change",material);
    input_std_precision(default_str,&(E->tracer.Melt_total_viscosity_change[material]),"0.0");
    sprintf(default_str,"Material_%d_Melt_frac_start_visc_change",material);
    input_std_precision(default_str,&(E->tracer.MeltF_start_visc_change[material]),"0.0");
    sprintf(default_str,"Material_%d_Melt_frac_end_visc_change",material);
    input_std_precision(default_str,&(E->tracer.MeltF_end_visc_change[material]),"0.0");

    sprintf(default_str,"Material_%d_dim_d0",material);  
    input_hgr_precision(default_str,&(E->tracer.dim_d0[material]),"0.0");
    sprintf(default_str,"Material_%d_dim_d1",material);  
    input_hgr_precision(default_str,&(E->tracer.dim_d1[material]),"0.0");
    sprintf(default_str,"Material_%d_dim_T0",material);  
    input_hgr_precision(default_str,&(E->tracer.dim_T0[material]),"0.0");
    sprintf(default_str,"Material_%d_dim_T1",material);  
    input_hgr_precision(default_str,&(E->tracer.dim_T1[material]),"0.0");
    sprintf(default_str,"Material_%d_dim_ag",material);  
    input_hgr_precision(default_str,&(E->tracer.dim_ag[material]),"0.0");
...
/* Depletion stuff - C. O'Neills work on melting*/
  ...
  if (E->tracer.Melt_on[material] > 0.5) {

  /*fprintf(stderr,"Did I get into melt or what??? Why? Melt_on = %g \n",E->tracer.Melt_on[material]) ;
  fprintf(stderr,"What is temp %g depth %g",E->tracer.T[m],E->tracer.tz[m]) ; */
     depth=E->tracer.dim_d0[material] + E->tracer.dim_d1[material]*E->tracer.tz[m];
     temp=E->tracer.dim_T0[material] + E->tracer.dim_T1[material]*E->tracer.T[m] + E->tracer.dim_ag[material]*depth;
  /* fprintf(stderr,"What is temp now %g E->tracer.T[m] %g \n",temp,E->tracer.T[m]) ; */
    /*temp=1;*/
    if (pressure[m]<=0) {
      fdep=0;
    }
    ...
    if (depcase==2){
      solidus[m]=E->tracer.sp0[material] + E->tracer.sp1[material]*depth+ E->tracer.sp2[material]*depth*depth + E->tracer.sp3[material]*depth*depth*depth;
      liquidus[m]=E->tracer.lp0[material] + E->tracer.lp1[material]*depth+ E->tracer.lp2[material]*depth*depth + E->tracer.lp3[material]*depth*depth*depth;
    } 
    else {
...
/*Is there more melt now than before? */

     if (fdep > E->tracer.melt[m] && fdep >  E->tracer.depl[m]) {
     	xdep=fdep;
     	/* Want to get dF/dt first */
     	xdep -= E->tracer.melt[m];
     	if (xdep > 0) {
	...
	/* Now update amount of melt present -latent heat done in energy solver*/
	E->tracer.melt[m]=fdep;
	 /* fprintf(stderr,"So what about Latent %g dFdot %g tracer.melt %g & depl %g sol %g temp %g depth %g \n",E->tracer.Latent_heat[material],E->tracer.dFdot[m],E->tracer.melt[m],E->tracer.depl[m],solidus[m],temp,E->tracer.tz[m]); */
	/* Do we now have more melt than ever before? If so increase depletion of residue */
	if (E->tracer.melt[m] > E->tracer.depl[m]) {
		E->tracer.depl[m]=E->tracer.melt[m];
	}
     
     
/*Is the percentage melt past the critical melt density for extraction? */

      if (E->tracer.melt[m] > E->tracer.Melt_extract[material]) {
        /* Set the degree of melt left to the critical density for melt extraction */
      	E->tracer.melt[m] = E->tracer.Melt_extract[material];
	/* Assume transported melt carried heat with it - reduce temp of residue down to supersolidus temp at critical extraction point */
	E->tracer.T[m] = solidus[m] + E->tracer.Melt_extract[material]*(liquidus[m] - solidus[m]);
      }

/*Work out what happens to viscosity for this melt density and depletion value */

     /* First depletion of residue increases residue viscosity */
     xdep = E->tracer.Visc[m];
     E->tracer.Visc[m] = xdep + xdep*E->tracer.Depl_visc_change[material]*E->tracer.depl[m];
     /* if (E->tracer.melt[m] > 0) {
     fprintf(stderr,"And now Visc 1 %g \n",E->tracer.Visc[m]);
	} */
     /* But the presence of melt decreases viscosity */
     xdep = E->tracer.Visc[m];
    /* if (E->tracer.melt[m] > 0) {
     fprintf(stderr,"Melt in else: Visc %g Melt %g Max_visc %g Visc_change %g \n",E->tracer.Visc[m],E->tracer.melt[m],E->tracer.MeltF_end_visc_change[material],E->tracer.Melt_total_viscosity_change[material]);
     }*/
     if (E->tracer.melt[m] > E->tracer.MeltF_start_visc_change[material] && E->tracer.melt[m] < E->tracer.MeltF_end_visc_change[material]) {
		 /* old function: E->tracer.Visc[m] = xdep - E->tracer.Melt_total_viscosity_change[material]*(E->tracer.melt[m] - E->tracer.MeltF_start_visc_change[material])/(E->tracer.MeltF_end_visc_change[material]  - E->tracer.MeltF_start_visc_change[material]);*/
		 /*Patrice substitued to the above the following melt dependent viscosity */
		 E->tracer.Visc[m] = xdep/pow(E->tracer.Melt_total_viscosity_change[material],((E->tracer.melt[m] - E->tracer.MeltF_start_visc_change[material])/(E->tracer.MeltF_end_visc_change[material] - E->tracer.MeltF_start_visc_change[material])));
		 /*fprintf(stderr,"Melt in if: Visc %g \n",E->tracer.Visc[m]); */		 
     }
     else if (E->tracer.melt[m] >= E->tracer.MeltF_end_visc_change[material]) {
     	E->tracer.Visc[m] = xdep - E->tracer.Melt_total_viscosity_change[material];
/*	fprintf(stderr,"Melt in else: Visc %g Melt %g \n",E->tracer.Visc[m],E->tracer.melt[m]);*/

     }
      /* if (E->tracer.melt[m] > 0) {
      	fprintf(stderr,"And now Visc 2 %g \n",E->tracer.Visc[m]);
	} */
 /*  Now outdated
...
#       E->tracer.Visc[m] = E->tracer.Visc[m]*(0.1);
     }
     else {
...
*/
/*    if (E->tracer.tx[m]>1.49 && E->tracer.tx[m]<1.51) {
      fprintf(E->fp1,"Temp Pressure solidus liquidus fdep %g %g %g %g %g \n",temp,pressure[m],solidus[m],liquidus[m],fdep);
    } */
	...
	/* C.O'N: adding strd_hor (horizontal deviatoric stress) and strd_ver (vertical) */
    E->tracer.strd[m] = E->tracer.Visc[m] * TrD[m];
    E->tracer.strd_hor[m] *= E->tracer.Visc[m];
    E->tracer.strd_ver[m] *= E->tracer.Visc[m]*(-1);
  ...
  if(1 /*&&  level == E->mesh.levmax*/) {
    ...
    if(E->control.print_convergence)
      fprintf(stderr,"%d tracers, %d: Visc min = %g, Visc max = %g\n",
	      E->tracer.NUM_TRACERS,level,minval,maxval);
    
    fprintf(E->fp,"Visc min = %g, Visc max = %g\n",minval,maxval);
  }
